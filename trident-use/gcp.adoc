...
| `serviceLevel` | 新卷的CVS-Performance或CVS服务级别。CVS-Performance值为 `standard`， `premium`或 `extreme`。CVS值为 `standardsw` 或 `zoneredundantstandardsw`。 | CVS-Performance默认值为"standard"。CVS默认值为"standardsw"。 
