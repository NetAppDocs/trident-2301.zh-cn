...
====
.<strong>使用NetApp Console管理 Kubernetes 集群</strong>
[%collapsible]
====
借助使用NetApp Console 的Astra Trident ，您可以升级到最新版本的Astra Trident，添加和管理存储类并将其连接到工作环境，以及使用Cloud Backup Service备份持久卷。控制台支持使用Trident操作员手动或使用 Helm 部署Astra Trident 。
